...
begin
  {Recupera o próximo código sequencial da entity Endereco)}
  try
    DataModuleWK.WKSequencial.Close;
...
begin
  {Recupera o próximo código sequencial da entity Endereco_Integracao(CEP)}
  try
    DataModuleWK.WKSequencial.Close;
  ...
  try
    ...
    try
      ...
      begin
        ...
        raise Exception.Create('Ocorreu problema na gravação. Entre em contato com o Suporte.'+cEOL+
...
begin
  ...
  //Exclui o CEP da pessoa para incluir novamente, podendo futuramente implementar a Pessoa ter vários CEPs: Residência, Trabalho, Casa de Praia e etc...
